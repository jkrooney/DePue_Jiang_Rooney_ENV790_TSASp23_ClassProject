...
plot(ts_electricity, col="red", ylab="Cents/kWh", xlab="Date", main="NC Residential Electricity Cost")
=======
>>>>>>> c9a0f9d404a11a59aff1d5265e61442498a762b3
...
natural_gas.df$UKRWAR <- factor(natural_gas.df$UKRWAR, levels = c(0, 1), labels = c("No", "Yes"))
...
plot(ts_electricity[,2], col="red", ylab="$/kWh", xlab="Date", main="NC Residential Electricity Cost")
...
# create covariates data frame for electricity (2001-2022)
...
fourier_train_gas <- fourier(window(ts_gas_equiv[, 2],
                                end = c(2022, 1)),
                         K = 6)

covariates_train_gas <- ts_gas_equiv[, 1] %>% 
  window(end = c(2022, 1)) %>% 
  cbind(window(ts_temperature, end = c(2022,1)
               ),
        fourier_train_gas)
...
# create covariates data frame for electricity (2001-2023)
fourier_full_gas <- fourier(window(ts_gas_equiv[, 2],
                                end = c(2023, 1)),
                         K = 6)

covariates_full_gas <- cbind(ts_gas_equiv[, 1], ts_temperature, fourier_full_gas)
...
=======
autoplot(ts_gas_equiv[,2]) +
  ylab("Price (cents/kWh") + 
  ggtitle("Natural Gas Price (cents/kWh)")
>>>>>>> c9a0f9d404a11a59aff1d5265e61442498a762b3


ts.plot(ts_gas_equiv[,2], ts_electricity[,2], gpars = list(col = c("red", "blue")), 
...
decomp_gas<-decompose(ts_gas_equiv[,2], type="multiplicative")
...
ts_gas_ns<-(ts_gas_equiv[,2] - decomp_gas$seasonal)
...
par(mfrow=c(1,2))
Acf(ts_gas_equiv[,2], lag.max=40, main="ACF Natural Gas")
Acf(ts_gas_ns, lag.max=40, main="ACF Non-Seasonal Gas")

par(mfrow=c(1,2))
Pacf(ts_gas_equiv[,2], lag.max=40, main="PACF Natural Gas")
...
#Gas SARIMA forecast
arima.gas.model<-auto.arima(window(ts_gas_equiv[, 2], end=c(2022,1)))
...
sarima_gas_perf <- ts_gas_equiv[, 2] %>% 
  window(start = c(2022, 2)) %>% 
...
arima.gas.four.forecast <- ts_gas_equiv[, 2] %>% 
  window(end = c(2022, 1)) %>% 
  auto.arima(seasonal = FALSE,
             xreg = fourier(window(ts_gas_equiv[, 2], end = c(2022, 1)), 
                            K = 6)
<<<<<<< HEAD
             )%>% 
...
=======
  ...
  forecast(xreg = fourier(window(ts_gas_equiv[, 2], 
                                 start = c(2022, 2)
>>>>>>> c9a0f9d404a11a59aff1d5265e61442498a762b3
                                 ),
                          K = 6), 
...
autoplot(arima.gas.four.forecast$mean, series = "ARIMA with Fourier Terms") +
  autolayer(ts_gas_equiv[, 2], series = "Historical Data") +
...
# model performance for gas data
arima_four_gas_perf <- ts_gas_equiv[, 2] %>% 
  window(start = c(2022, 2)) %>% 
...
# STL for gas
stl.gas.forecast <- ts_gas_equiv[, 2] %>% 
  window(end = c(2022, 1)) %>% 
  stlf(h = 12)

autoplot(ts_gas_equiv[, 2], series = "Historical Data") +
...
nn.gas.forecast <- ts_gas_equiv[, 2] %>% 

nn.gas.forecast <- ts_gas_equiv %>% 
  window(end = c(2022 ,1)) %>% 
  nnetar(p = 2, P = 1,
         xreg = fourier(window(ts_gas_equiv[, 2], 
                               end = c(2022, 1)
                               ),
                        K = 6)
         ) %>% 
...
# neural network forecast for gas data
nn.cov.gas.forecast <- ts_gas_equiv[, 2] %>% 
  window(end = c(2022 ,1)) %>% 
  nnetar(p = 2, P = 1,
...
# neural network model performance for gas data
nn_cov_gas_perf <- ts_gas_equiv[, 2] %>% 
  window(start = c(2022, 2)) %>% 
...
tbats.gas.forecast <- ts_gas_equiv[, 2] %>% 
  window(end = c(2022, 1)) %>% 
  tbats() %>% 
  forecast(h = 12)
...
gas.forecast.2324 <- ts_gas_equiv[, 2] %>% 
  stlf(h = 12)
...
=======
   autolayer(window(ts_gas_equiv[, 2], start = c(2017, 1)), 
            series = "historical data for natural gas") +
  theme_classic() +
