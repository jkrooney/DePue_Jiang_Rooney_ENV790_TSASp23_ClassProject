...
```{r create time series for natural gas, echo=FALSE}
#create timeseries for natural gas

ts_NG<-ts(na.omit(natural_gas.df[,2]), start=c(1989,1), frequency=12)
```

```{r plot initial time series, echo=FALSE, fig.height=4}
# plot raw time series
plot(ts_NG, col="blue", ylab="$/Mcf", xlab="Date", main="Figure 1: NC Residential Natural Gas Cost")
```
...
#ts_diff<-ts(nc_energy.df$cost_diff, start= c(2001,1), frequency=12)

#plot(ts_electricity, col="red", ylab="Price Difference (Cents/kWh)", xlab="Date", main="Direct Price Difference (Electricity price minus gas price)")

```

```{r Electric Plot, echo=FALSE, fig.height=4}
plot(ts_electricity, col="red", ylab="Price (cents/kWh)", xlab="Date", main="Figure 2: NC Residential Electricity Cost")

```


```{r plot elec and gas ts, echo=FALSE, fig.height=4}
#plot both electric & gas TS together
ts.plot(ts_gas_equiv, ts_electricity, gpars = list(col = c("red", "blue")), 
        xlab="Date", ylab="cents/kWh", 
        main="Figure 3: Comparison of Natural Gas and Electricity Costs")
...
```{r gas decomp plot, echo=FALSE, fig.height=4}


ts_gas_equiv %>% 
  window(start = c(2013,1), end = c(2023,1)) %>%
  stl(s.window = "periodic") %>% 
  plot( main="Figure 4: Decomposition of Natural Gas Prices  Time Series")

```
```{r electric decomp, echo=FALSE, fig.height=4}
ts_electricity[,1] %>% 
  window(start = c(2013,1), end = c(2023,1)) %>%
  stl(s.window = "periodic") %>% 
  plot( main="Figure 5: Decomposition of Electricity Prices Time Series")

```



```{r ACF and PACF of electricity and natural gas, echo=FALSE, fig.height=8}

# ACF and PACF of electricity data
par(mfrow=c(2,2))
Acf(ts_electricity[,1], lag.max=40, main="Fig. 6: ACF Electricity")
Acf(ts_elec_ns, lag.max=40, main="Fig. 7: ACF Non-Seasonal Electricity")
Pacf(ts_electricity[,1], lag.max=40, main="Fig. 8: PACF Electricity")
Pacf(ts_elec_ns, lag.max=40, main="Fig. 9: PACF Non-Seasonal Electricity")

# ACF and PACF of natural gas data
par(mfrow=c(2,2))
Acf(ts_gas_equiv, lag.max=40, main="Fig. 10: ACF Natural Gas")
Acf(ts_gas_ns, lag.max=40, main="Fig. 11: ACF Non-Seasonal Gas")
Pacf(ts_gas_equiv, lag.max=40, main="Fig. 12: PACF Natural Gas")
Pacf(ts_gas_ns, lag.max=40, main="Fig. 13: PACF Non-Seasonal Gas")
```
...
```{r Seasonal Arima, echo=FALSE, fig.height=4}
...
autoplot(ts_electricity) +
  ...
  ylab("Price (cents/kWh)") +
  ggtitle("Figure 14: Electricity - SARIMA")
...
autoplot(ts_gas_equiv)+
  ...
  ylab("Price (cents/kWh)") +
  ggtitle("Figure 15: Natural Gas - SARIMA")
  # performance is not that good
```
...
```{r Arima with Fourier terms, echo=FALSE, fig.height=4}
...
autoplot(arima.e.four.forecast$mean, series = "ARIMA with Fourier Terms") +
  ...
  ggtitle("Figure 16: Electricity - ARIMA with Fourier Terms") +
  ylab("Price (cents/kWh)")
...
autoplot(arima.gas.four.forecast$mean, series = "ARIMA with Fourier Terms") +
  ...
  ggtitle("Figure 17: Natural Gas - ARIMA with Fourier Terms") +
  ylab("Price (cents/kWh)")
...
autoplot(stl.e.forecast$mean, series = "STL + ETS") +
  autolayer(ts_electricity, series = "Historical Data") +
  ggtitle("Figure 18: Electricity - STL + EST") +
  ylab("Price (cents/kWh)")
...
autoplot(ts_gas_equiv, series = "Historical Data") +
  ...
  ggtitle("Figure 19: Natural Gas - STL + EST") +
  ylab("Price (cents/kWh)")
```
...
autoplot(nn.e.forecast$mean, series = "Neural Network") +
  autolayer(ts_electricity, series = "Historical Data") +
  ggtitle("Figure 20: Electricity - Neural Network") +
  ylab("Price (cents/kWh)")
...
autoplot(nn.gas.forecast$mean, series = "Neural Network") +
  autolayer(ts_gas_equiv, series = "Historical Data") +
  ggtitle("Figure 21: Natural Gas - Neural Network") +
...
autoplot(tbats.e.forecast$mean, series = "TBATS") +
  autolayer(ts_electricity, series = "Historical Data") +
  ggtitle("Figure 22: Electricity - TBATS") +
  ylab("Price (cents/kWh)")
```
...
autoplot(tbats.gas.forecast$mean, series = "TBATS") +
  autolayer(ts_gas_equiv, series = "Historical Data") +
  ggtitle("Figure 23: Natural Gas - TBATS") +
  ylab("Price (cents/kWh equiv)")
```
&nbsp;
&nbsp;
...
autoplot(nn.cov.e.forecast$mean, series = "Neural Network") +
  ...
  ggtitle("Figure 24: Electricity - Neural Network") +
  ylab("Price (cents/kWh)")
...
autoplot(nn.cov.gas.forecast$mean, series = "Neural Network") +
  ...
  ggtitle("Figure 25: Natural Gas - Neural Network") +
  ylab("Price (cents/kWh)")
```
...
autoplot(sarima.e.cov.forecast$mean, series = "ARIMA with Covariates") +
  autolayer(ts_electricity_war[, 2], series = "Historical Data") +
  ggtitle("Figure 26: Electricity - ARIMA with Covariates") +
  ylab("Price (cents/kWh)")
...
autoplot(sarima.gas.cov.forecast$mean, series = "SARIMA with Covariates") +
  autolayer(ts_gas_equiv_war[, 1], series = "Historical Data") +
  ylab("Price (cents/kWh)") +
  ggtitle("Figure 27: Natural Gas - SARIMA with Covariates") +
...
```{r compare performace scores and generate tables for use, echo=FALSE, warning=FALSE, fig.height=4}
# plot these together
ts_electricity %>% 
  ...
  ylab("Price (cents/kWh)") +
  ggtitle("Figure 28: Comparision of Modeling Methods - Electricity") +
  theme_classic()
...
ts_gas_equiv %>% 
  ...
  ylab("Price (cents/kWh)") +
  ggtitle("Figure 29: Comparision of Modeling Methods - Natural Gas") +
  theme_classic()
...
e.forecast.2324$mean %>% 
  ...
  theme_classic() +
  ggtitle("Figure 30: 12 Month STL Forecast of Electricity and Natural Gas Price") +
  ylab("Price (cents/kWh)")
